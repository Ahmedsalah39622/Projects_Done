...
Public Class Frm_addTimevalue
    ...
    Sub Find_all()
        ...
        sql = "SELECT        dbo.Vw_TimeSeting_Value.Code_Move, dbo.Vw_TimeSeting_Value.NameDir, dbo.Vw_TimeSeting_Value.NameDeprt, dbo.Vw_TimeSeting_Value.Name, dbo.Vw_TimeSeting_Value.Emp_Name, dbo.Vw_TimeSeting_Value.DateIn, " & _
        "                         dbo.vw_TimeSeting_Value2.DateIn AS DateOut, dbo.Vw_TimeSeting_Value.Time_in, dbo.Vw_TimeSeting_Value.Time_out, dbo.vw_TimeSeting_Value2.ValueTime  " & _
        " FROM            dbo.Vw_TimeSeting_Value LEFT OUTER JOIN " & _
        "                         dbo.vw_TimeSeting_Value2 ON dbo.Vw_TimeSeting_Value.Code_Move = dbo.vw_TimeSeting_Value2.Code_Move  " & _
        "         "
        ...
        GridView2.Columns(9).Caption = "قيمة الساعة الاضافى"

        'GridView2.Columns(10).Caption = "عدد الايام"
  ...
  " HAVING        (dbo.BD_DIRCTORAY.Name like '%" & Com_Dir.Text & "%') AND (dbo.BD_DIRCTORAY.Compny_Code = '" & VarCodeCompny & "') AND (dbo.BD_DEPARTMENTS.Name like '%" & Com_Deprt.Text & "%')  AND (dbo.BD_JOBNAMES.Name like '%" & Com_JopName.Text & "%') "
    ...
    Sub last_Data()
        On Error Resume Next
        sql = "  SELECT        MAX(Code_Move) AS MaxMaim,Compny_Code FROM            Seting_TimeValue    GROUP BY Compny_Code  HAVING        (MAX(Code_Move) IS NOT NULL) and (Compny_Code = '" & VarCodeCompny & "')  "
        rs = Cnn.Execute(sql)
    ...
    Private Sub cmd_save_Click(sender As Object, e As EventArgs) Handles cmd_save.Click
        ...
        If Len(TXt_valueTime.Text) = 0 Then MsgBox("من فضلك ادخل قيمة الساعة ", MsgBoxStyle.Critical + MsgBoxStyle.MsgBoxRtlReading, "Css Solution Software Module") : Exit Sub
    ...
    Sub add_payment()
        ...
        If vardayrent <> vardayrent2 And oldDay <> 1 Then oldDay = oldDay - 1
        ...
        While (CurrD < endP)
            ...
            sql = "INSERT INTO Seting_TimeValue (Code_Move, Date_From,code_dir,code_deprt,code_jop,emp_code,Time_in,Time_out,Compny_Code,ValueTime) " & _
            " values  ('" & com_Code.Text & "' ,'" & vardayrent3 & "','" & VarcodeDir & "','" & Varcodedeprt & "','" & varcode_jop & "','" & Varcode_emp & "','" & TimeEdit1.Text & "','" & TimeEdit2.Text & "','" & VarCodeCompny & "','" & TXt_valueTime.Text & "' )"
            Cnn.Execute(sql)
    ...
    Sub find_data()
        ...
        sql2 = "SELECT        dbo.Vw_TimeSeting_Value.Code_Move, dbo.Vw_TimeSeting_Value.NameDir, dbo.Vw_TimeSeting_Value.NameDeprt, dbo.Vw_TimeSeting_Value.Name, dbo.Vw_TimeSeting_Value.Emp_Name, dbo.Vw_TimeSeting_Value.DateIn, " & _
        "                         dbo.vw_TimeSeting_Value2.DateIn AS DateOut, dbo.Vw_TimeSeting_Value.Time_in, dbo.Vw_TimeSeting_Value.Time_out, dbo.vw_TimeSeting_Value2.ValueTime  " & _
        " FROM            dbo.Vw_TimeSeting_Value LEFT OUTER JOIN " & _
        "                         dbo.vw_TimeSeting_Value2 ON dbo.Vw_TimeSeting_Value.Code_Move = dbo.vw_TimeSeting_Value2.Code_Move  " & _
        "        WHERE Vw_TimeSeting_Value.Code_Move ='" & com_Code.Text & "' "
        ...
        GridView1.Columns(9).Caption = "قيمة الساعة الاضافى"
    ...
    Sub fill_data()
        ...
        "                         dbo.vw_TimeSeting_Value2.DateIn AS DateOut, dbo.Vw_TimeSeting1.Time_in, dbo.Vw_TimeSeting1.Time_out  " & _
        " FROM            dbo.Vw_TimeSeting1 LEFT OUTER JOIN  " & _
        "                         dbo.vw_TimeSeting_Value2 ON dbo.Vw_TimeSeting1.Code_Move = dbo.vw_TimeSeting_Value2.Code_Move AND dbo.Vw_TimeSeting1.Compny_Code = dbo.vw_TimeSeting_Value2.Compny_Code  " & _
    ...
    Private Sub Cmd_Delete_Click(sender As Object, e As EventArgs) Handles Cmd_Delete.Click
        ...
        Select Case x
            ...
            Case vbYes
                sql = "Delete Seting_TimeValue  WHERE Code_Move = " & com_Code.Text & " and Compny_Code ='" & VarCodeCompny & "' "
                rs = Cnn.Execute(sql)
    ...
    Private Sub GridControl1_DoubleClick(sender As Object, e As EventArgs) Handles GridControl1.DoubleClick
        ...
        "                         dbo.vw_TimeSeting_Value2.DateIn AS DateOut, dbo.Vw_TimeSeting1.Time_in, dbo.Vw_TimeSeting1.Time_out  " & _
        " FROM            dbo.Vw_TimeSeting1 LEFT OUTER JOIN  " & _
        "                         dbo.vw_TimeSeting_Value2 ON dbo.Vw_TimeSeting1.Code_Move = dbo.vw_TimeSeting_Value2.Code_Move   " & _
        " WHERE         (dbo.Vw_TimeSeting1.Code_Move = '" & value2 & "')  "
