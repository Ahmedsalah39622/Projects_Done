...
Public Class frm_SalfaHR
 ...
 "        dbo.TB_HR_SalfSalary.Flag_Discount, dbo.TB_HR_SalfSalary.Resone_Discount "
        ...
        GridView1.Columns(3).Caption = "تاريخ السلفة"
        GridView1.Columns(4).Caption = "قيمة السلفة"
        GridView1.Columns(5).Caption = "الحالة"
        GridView1.Columns(6).Caption = "سبب السلفة"
    ...
    Sub last_Data()
        ...
        'On Error Resume Next
        sql = "SELECT        MAX(No_Order) AS MaxmamNo  FROM   dbo.TB_HR_SalfSalary   HAVING(MAX(No_Order) Is Not NULL)  "
    ...
    Private Sub Cmd_save_Click(sender As Object, e As EventArgs) Handles Cmd_save.Click
        ...
        If Len(txt_DiscountValue.Text) = 0 Then MsgBox("من فضلك ادخل قيمة السلفة ", MsgBoxStyle.Critical + MsgBoxStyle.MsgBoxRtlReading, "Css Solution Software Module") : Exit Sub
        ...
        sql = "SELECT *  FROM TB_HR_SalfSalary where No_Order =" & txt_NoOrder.Text & "  "
        rs2 = Cnn.Execute(sql)
        If rs2.EOF = False Then MsgBox("رقم تم تكرار", MsgBoxStyle.Critical, "Cerative Smart Software") : Exit Sub



        sql = "INSERT INTO TB_HR_SalfSalary (No_Order, Code_Emp,Date_Entry,Value_Discount,Resone_Discount) " & _
            " values  (" & txt_NoOrder.Text & " ,N'" & txt_EmpNo.Text & "',N'" & vardate7 & "',N'" & txt_DiscountValue.Text & "',N'" & txt_Notes.Text & "'  )"
        Cnn.Execute(sql)
    ...
    Sub find_All_Name()
        ...
        sql2 = "  SELECT dbo.TB_HR_SalfSalary.No_Order, dbo.TB_HR_SalfSalary.Code_Emp, dbo.Emp_employees.Emp_Name, dbo.TB_HR_SalfSalary.Date_Entry, dbo.TB_HR_SalfSalary.Value_Discount, " & _
  "                 iif( dbo.TB_HR_SalfSalary.Flag_Discount=1,'معتمد','غير معتمد') as status, dbo.TB_HR_SalfSalary.Resone_Discount, dbo.BD_DIRCTORAY.Name AS Dir, dbo.BD_DEPARTMENTS.Name AS Deprt, dbo.BD_DEPARTMENTS.Name, dbo.BD_JOBNAMES.Name as jopnmae " & _
  " FROM     dbo.TB_HR_SalfSalary INNER JOIN " & _
  "                  dbo.Emp_employees ON dbo.TB_HR_SalfSalary.Code_Emp = dbo.Emp_employees.Emp_Code  " & _
  "                   INNER JOIN " & _
  ...
  "                dbo.BD_JOBNAMES ON dbo.Emp_employees.Emp_Code_JopName = dbo.BD_JOBNAMES.Code " & _
  " GROUP BY dbo.TB_HR_SalfSalary.No_Order, dbo.TB_HR_SalfSalary.Code_Emp, dbo.Emp_employees.Emp_Name, dbo.TB_HR_SalfSalary.Date_Entry, dbo.TB_HR_SalfSalary.Value_Discount,  " & _
  "        dbo.TB_HR_SalfSalary.Flag_Discount, dbo.TB_HR_SalfSalary.Resone_Discount, dbo.BD_DIRCTORAY.Name, dbo.BD_DEPARTMENTS.Name, dbo.BD_DEPARTMENTS.Name, dbo.BD_JOBNAMES.Name having dbo.TB_HR_SalfSalary.No_Order ='" & GridView1.GetRowCellValue(visibleRowHandle, GridView1.Columns(0)) & "' "
    ...
    Private Sub Cmd_Delete_Click(sender As Object, e As EventArgs) Handles Cmd_Delete.Click
        sql = "select * from  TB_HR_SalfSalary WHERE No_Order = '" & txt_NoOrder.Text & "' and Flag_Discount = 1 "
        rs = Cnn.Execute(sql)
        ...
        Select Case x
            ...
            Case vbYes
                sql = "Delete TB_HR_SalfSalary  WHERE No_Order = '" & txt_NoOrder.Text & "' "
                rs = Cnn.Execute(sql)
