...
Public Class Frm_CompnyInfo2
    Dim varcodecity, varcodeRegion As Integer
    Dim valueNamePersonal As String
    Sub last_Data()
    ...
    Sub clear()
        ...
        txt_Notes.Text = ""
        txt_Reigon.Text = ""
        txt_Commercial_Register.Text = ""
        txt_Registration_number.Text = ""
        txt_Taxcard.Text = ""
        txt_ImportCard.Text = ""
        txt_NoTax.Text = ""
    ...
    End Sub
    Sub find_pesonal()
        If txt_Code.Text = "" Then Exit Sub
        sql = "select Compny_PhonePersonal,Compny_NamePersonal from St_CompnyInfoOnPrsonal where Compny_Code ='" & txt_Code.Text & "'"
        ...
        GridView6.Columns(0).Caption = "رقم تليفون المتعامل"
        GridView6.Columns(1).Caption = " أسم المتعامل "
   
        GridView6.Columns(0).Width = "200"
        GridView6.Columns(1).Width = "300"
        ds = Nothing
        da = Nothing
        con.Close()
        con.Dispose()
    ...
    Private Sub GridControl2_DoubleClick(sender As Object, e As EventArgs) Handles GridControl2.DoubleClick
        ...
        clear()
        '===================================Find
        sql2 = "  SELECT St_CompnyInfo.Compny_Code, St_CompnyInfo.Compny_Name, BD_CITIES.Name AS NameCity, BD_REGION.Name AS NameRegion, St_CompnyInfo.Activity, St_CompnyInfo.Compny_Address  " & _
        "         , St_CompnyInfo.Compny_Phone1, St_CompnyInfo.Compny_Fax, St_CompnyInfo.Compny_Website, St_CompnyInfo.Compny_Email,   " & _
        "   St_CompnyInfo.Compny_Phone2, St_CompnyInfo.Compny_Box, St_CompnyInfo.Notes, St_CompnyInfo.Compny_CommercialRegister, St_CompnyInfo.Compny_RegistrationNumber, St_CompnyInfo.Compny_Taxcard, St_CompnyInfo.Compny_ImportCard, St_CompnyInfo.Compny_NoTax, St_CompnyInfo.Compny_owner  " & _
        " FROM     BD_REGION INNER JOIN  " & _
        "                 BD_CITIES ON BD_REGION.Code = BD_CITIES.Code INNER JOIN  " & _
        ...
        If rs2.EOF = False Then
            txt_Code.Text = rs2("Compny_Code").Value
            txt_name.Text = rs2("Compny_Name").Value
            txt_CityName.Text = rs2("NameCity").Value
            txt_Reigon.Text = rs2("NameRegion").Value
            txt_Activety.Text = rs2("Activity").Value
            txt_Address.Text = rs2("Compny_Address").Value
            txt_Phone1.Text = rs2("Compny_Phone1").Value
            txt_Phone2.Text = rs2("Compny_Phone2").Value
            txt_fax.Text = rs2("Compny_Fax").Value
            txt_Website.Text = rs2("Compny_Website").Value
            txt_Email.Text = rs2("Compny_Email").Value
            txt_Box.Text = rs2("Compny_Box").Value
            txt_Notes.Text = rs2("Notes").Value
            txt_Commercial_Register.Text = rs2("Compny_CommercialRegister").Value
            txt_Registration_number.Text = rs2("Compny_RegistrationNumber").Value
            txt_Taxcard.Text = rs2("Compny_Taxcard").Value
            txt_ImportCard.Text = rs2("Compny_ImportCard").Value
            txt_NoTax.Text = rs2("Compny_NoTax").Value

            txt_owner.Text = rs2("Compny_owner").Value
    ...
    Private Sub Cmd_Save_Click_1(sender As Object, e As EventArgs) Handles Cmd_Save.Click
        ...
        Else
            ...
            '            Compny_CommercialRegister	nvarchar(30)	Checked
            'Compny_RegistrationNumber	nvarchar(40)	Checked
            'Compny_Taxcard	nvarchar(40)	Checked
            'Compny_ImportCard	nvarchar(40)	Checked
            'Compny_NoTax	nvarchar(40)	Checked
            'Compny_Peronal_No	nvarchar(40)	Checked
            'Compny_owner	nvarchar(100)	Checked
            Dim sql2 As String
            sql2 = "INSERT INTO St_CompnyInfo (Compny_Code, Compny_Name,Code_City,Code_Regione,Activity,Compny_Address,Compny_Logo,Compny_Phone1,Compny_Phone2,Compny_Fax,Compny_Box,Compny_Website,Compny_Email,Notes,Compny_CommercialRegister,Compny_RegistrationNumber,Compny_Taxcard,Compny_ImportCard,Compny_NoTax,Compny_owner) " & _
              " values  ('" & txt_Code.Text & "' ,'" & txt_name.Text & "','" & varcodecity & "','" & varcodeRegion & "','" & txt_Activety.Text & "','" & txt_Address.Text & "','" & txt_picpath.Text & "','" & txt_Phone1.Text & "','" & txt_Phone2.Text & "','" & txt_fax.Text & "','" & txt_Box.Text & "','" & txt_Website.Text & "','" & txt_Email.Text & "','" & txt_Notes.Text & "','" & txt_Commercial_Register.Text & "','" & txt_Registration_number.Text & "','" & txt_Taxcard.Text & "','" & txt_ImportCard.Text & "','" & txt_NoTax.Text & "','" & txt_owner.Text & "')"
            rs = Cnn.Execute(sql2)
    ...
    Private Sub cmd_save2_Click(sender As Object, e As EventArgs) Handles cmd_save2.Click
        If Len(txt_name.Text) = 0 Then MsgBox("من فضلك أختار اسم الشركة ", MsgBoxStyle.Critical + MsgBoxStyle.MsgBoxRtlReading, "Css Solution Software Module") : Exit Sub
        If Len(txt_PhonePersonal.Text) = 0 Then MsgBox(" من فضلك ادخل رقم تليفون المتعامل ", MsgBoxStyle.Critical + MsgBoxStyle.MsgBoxRtlReading, "Css Solution Software Module") : Exit Sub
        If Len(txt_NamePersonal.Text) = 0 Then MsgBox("من فضلك ادخل أسم المتعامل مع الشركة ", MsgBoxStyle.Critical + MsgBoxStyle.MsgBoxRtlReading, "Css Solution Software Module") : Exit Sub


        sql2 = "INSERT INTO St_CompnyInfoOnPrsonal (Compny_Code, Compny_PhonePersonal,Compny_NamePersonal) " & _
  " values  ('" & txt_Code.Text & "' ,'" & txt_PhonePersonal.Text & "','" & txt_NamePersonal.Text & "')"
        rs = Cnn.Execute(sql2)
        MsgBox("تم حفظ البيانات", MsgBoxStyle.Information + MsgBoxStyle.MsgBoxRight, "Css Solution Software Module")
        find_pesonal()
    End Sub
    ...
    Private Sub GridControl3_DoubleClick(sender As Object, e As EventArgs) Handles GridControl3.DoubleClick
        ...
        txt_PhonePersonal.Text = GridView6.GetRowCellValue(visibleRowHandle, GridView6.Columns(0))
        txt_NamePersonal.Text = GridView6.GetRowCellValue(visibleRowHandle, GridView6.Columns(1))
        valueNamePersonal = GridView6.GetRowCellValue(visibleRowHandle, GridView6.Columns(1))
    End Sub

    Private Sub cmd_update2_Click(sender As Object, e As EventArgs) Handles cmd_update2.Click
        sql = "UPDATE  St_CompnyInfoOnPrsonal  SET Compny_PhonePersonal ='" & txt_PhonePersonal.Text & "',Compny_NamePersonal ='" & txt_NamePersonal.Text & "'  WHERE Compny_Code = '" & txt_Code.Text & "' and Compny_NamePersonal ='" & valueNamePersonal & "' "
        rs = Cnn.Execute(sql)
    ...
    Private Sub cmd_Delete2_Click(sender As Object, e As EventArgs) Handles cmd_Delete2.Click
        ...
        Select Case x
            ...
            Case vbYes
                sql = "Delete St_CompnyInfoOnPrsonal  WHERE Compny_Code = '" & txt_Code.Text & "' and Compny_NamePersonal ='" & valueNamePersonal & "' "
                rs = Cnn.Execute(sql)
    ...
    Private Sub txt_Reigon_ButtonClick1(sender As Object, e As XtraEditors.Controls.ButtonPressedEventArgs) Handles txt_Reigon.ButtonClick
    ...
    Private Sub txt_Reigon_EditValueChanged_1(sender As Object, e As EventArgs) Handles txt_Reigon.EditValueChanged
